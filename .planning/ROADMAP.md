...
**Success Criteria** (what must be TRUE):
  1. Toàn bộ corpus ~60K ảnh (Snapshot Serengeti có bbox + Caltech Camera Traps giữ hoàn toàn ngoài tập train) đã tải, chuẩn hoá, đóng gói dạng shard và copy về đĩa local của session lúc khởi động — không đọc trực tiếp từng file nhỏ trên Drive.
...
**Success Criteria** (what must be TRUE):
  ...
  2. Bảng ablation cộng dồn (gốc → +H1 → +H1+H2 → +full) không còn ô trống, mọi ô có bootstrap CI; bảng cái giá chuyên biệt hoá tồn tại trên cả hai trục — miền tổng quát (COCO/Kodak) và site mới giữ ngoài (CCT).
